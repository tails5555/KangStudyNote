...
ArrayList의 크기가 동적으로 추가되거나 삭제할 때 그 인덱스 다음에 있는 데이터를 밀거나 당기는 경우의 성능 저하의 발생 여부로 LinkedList보다 느릴 수도 있다는 사실을 생각해봐야 한다. 이 때 내부에서 이용하는 메소드가 바로 `System.arraycopy()`이다. 

또한 ArrayList 내부에 배열이란 자체가 존재해서 인덱스를 이용할 때 더욱 효과적인 List로도 뽑힌다.

> **`System.arraycopy()`**
> 
> 이 개념은 `C`, `C++`에서 `memcpy()` 함수와 같은 개념과 유사하다. 배열을 복사할 때 반복문을 쓰거나 `clone()` 함수를 이용할 때 쓸데 없는 인스턴스를 생성할 필요가 없게 되어 메모리 자원 낭비를 막을 수 있고 더 빠른 속도를 보장할 수 있다.
...
> 
> 그렇지만 데이터의 양이 너무 많아진다면 LinkedList를 이용하는 것이 좋다. ArrayList는 데이터가 많아지면 일일히 배열에 저장하기 때문에 공간 복잡도가 O(N)으로 커지기 때문이다. 반대로 데이터 양이 적으면 ArrayList를 쓰는 것이 효율적일 수도 있다. LinkedList의 Node 수가 극히 적다면 ArrayList와 다르게 공간 복잡도가 커질 수도 있기 때문이다.
> 
